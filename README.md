...
เลือก **5) Backup data ลง Download ของมือถือ** แล้วตั้งชื่อ หรือกด Enter เพื่อใช้วันที่และเวลา สคริปต์จะสร้าง `data-*.zip` ใน **ที่เก็บข้อมูลภายในเครื่อง → Download** (โดยทั่วไปคือ `/storage/emulated/0/Download`) ใช้โฟลเดอร์เดียวกับเมนู 3 ที่นำเข้าจากมือถือ โดยคัดลอก `~/SillyTavern/data` ทั้งโฟลเดอร์ ไม่ต้องใช้ Material Files และไม่ย้ายข้อมูลต้นฉบับ

`~/storage/downloads` ที่ Termux สร้างให้เป็นทางลัดไปยัง Download ของมือถือ สคริปต์จะแสดงพาธจริงของปลายทาง และไม่ใช้โฟลเดอร์ธรรมดาที่สร้างไว้ภายใน Termux แทน ส่วน **เมนู 1** ยังเป็นการสำรองภายใน Termux ที่ `~/ST-Backups`
...
ไฟล์ ZIP จะถูกตรวจสอบก่อนบันทึกสำเร็จ หากชื่อซ้ำจะถามก่อนเขียนทับ และเก็บไฟล์เดิมไว้จนกว่าจะสร้างไฟล์ใหม่สำเร็จ เมื่อต้องการกู้คืน เลือก **3) นำเข้า Backup ZIP จาก Downloads (ทุกชื่อ)** แล้วใช้ **2) Restore Backup** (เมนูนำเข้าจะย้าย ZIP เข้า `~/ST-Backups`)

เมนูนำเข้ารองรับ ZIP ทุกชื่อ เช่น `backup.zip`, `my-st.zip` หรือ `สำรองข้อมูล.ZIP` รวมชื่อที่มีช่องว่างและนามสกุลตัวพิมพ์ใหญ่/เล็ก หลังเลือกไฟล์ ระบบจะตรวจว่า ZIP สมบูรณ์และมีโครงสร้าง `data/` ที่รองรับก่อนนำเข้า ไฟล์ที่ตรวจไม่ผ่านจะยังอยู่ใน Download

ระหว่างสร้าง ตรวจสอบ และแตก ZIP จะแสดงเปอร์เซ็นต์ตามปริมาณข้อมูลที่ 7-Zip ประมวลผลจริงในแต่ละขั้น (ไม่ใช่เปอร์เซ็นต์รวมทุกขั้น และไม่ได้นำขนาด ZIP ที่บีบอัดแล้วมาหารขนาดต้นฉบับ) ไฟล์เล็กอาจเสร็จทันที ส่วนการคัดลอกโฟลเดอร์จาก installer และย้ายไฟล์ยังใช้แถบเคลื่อนไหวพร้อมสถานะสำเร็จ/ล้มเหลวตามเดิม

ใช้แพ็กเกจ `7zip` (คำสั่ง `7zz`) และ `unzip` โดยสคริปต์จะเสนอให้ติดตั้งส่วนที่ขาดผ่าน `pkg install` ไฟล์สำรองยังเป็น `.zip` ที่ใช้เมนู Restore เดิมได้ ดู [แพ็กเกจ 7-Zip ของ Termux](https://github.com/termux/termux-packages/blob/master/packages/7zip/build.sh)
